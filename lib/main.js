...
function buildSplitterSupplyChain() {
  red_crate  = buildCrate(600,280);
  blue_crate = buildCrate(600,40);

  var machine = buildMachine("purple", 400,225);

  machine.deliverable = function() {
    return([newWidget("red"), newWidget("blue")]);
  }

  machines.push(machine);

  var splitter = buildSplitter(machine);

  var red_crate_order = splitter.subOrder(red_crate);
  
  // FIXME:: BUILD A CONSUMER FUNCTION
  red_crate.consume = function() {
    w = red_crate.pop();
    stage.removeChild(w);;
    red_crate_order.submit();
  ...
  _.times(3, function() { red_crate_order.submit(); });

  var blue_crate_order = splitter.subOrder(blue_crate);

  // FIXME:: BUILD A CONSUMER FUNCTION
  blue_crate.consume = function() {
    w = blue_crate.pop();
    stage.removeChild(w);;
    blue_crate_order.submit();
  ...
  t.makeInteractive(red_crate)

  red_crate.tap = function() {
    var r = red_crate.pop();
  ...
  blue_crate.tap = function() {
    var r = blue_crate.pop();
  ...
  _.times(3, function() { blue_crate_order.submit(); });
