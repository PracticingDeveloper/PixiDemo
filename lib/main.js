...
var purifier_machine, combiner_machine;
...
function buildPurifierSupplyChain() {
  ...
  purifier_machine = buildMachine("blue", 400,400);
  var order   = buildOrder(purifier_machine, blue_crate);
  
  purifier_machine.worker.delay = 4500
...
function buildMachineSupplyChain() {
  combiner_machine = buildMachine("purple", 750,250);
  ...
  combiner_machine.worker.delay = 2500
...
function gameLoop() {
  ...
  if (purifier_machine.on) {
    purifier_machine.gear.rotation += 0.1
  }

  if (combiner_machine.on) {
    combiner_machine.gear.rotation += 0.1
  }
